...
class LiveView {
  constructor() {
    ...
    this.lastError = null;
    this._restartDelay = 1200;   // current backoff delay in ms
    this._restartTimer = null;   // pending restart timer
  }
  ...
  start() {
    ...
    if (SIMULATE) {
      ...
      log('live view started (simulated)');
      return;
    }

    // --set-config output=PC asks the EOS for its largest preview size.
    // It is best-effort: models that lack the property just ignore it.
    const args = ['--set-config', 'output=PC', '--capture-movie', '--stdout'];
    log('starting live view: gphoto2', args.join(' '));
    ...
    child.stdout.on('data', (chunk) => {
      // At least one frame arrived — camera is alive, reset backoff.
      this._restartDelay = 1200;
      this.ingest(chunk);
    });
    ...
    child.on('exit', (code, signal) => {
      log(`live view exited code=${code} signal=${signal || '-'}`);
      this.child = null;
      this.buffer = Buffer.alloc(0);
      ...
      // Auto-restart only while somebody is still watching and we did not stop
      // on purpose (a capture stop sets wantRunning=false first).
      if (this.wantRunning && this.subscribers.size > 0) {
        // When the camera is physically absent, back off more aggressively so we
        // don't spam gphoto2 calls every 50 ms. Each successive failure doubles
        // the delay (capped at 30 s); a successful run resets it to 1.2 s.
        const delay = cameraNotFound
          ? Math.min(this._restartDelay * 2, 30000)
          : 1200;
        this._restartDelay = delay;
        log(`scheduling live view restart in ${(delay / 1000).toFixed(1)}s`);
...
async function captureStill() {
  ...
  const hadViewers = liveView.subscribers.size > 0;
  ...
  try {
    const result = await run(
      ...
      [
        ...
        target,
        '--force-overwrite',
        // Keep the file off the SD card so repeated sessions do not fill it and
        // so we never race the camera's own file numbering.
        '--set-config',
        'capturetarget=0',
      ],
      { timeout: 45000 }
    ...
    if (!fs.existsSync(target)) {
      ...
      } else if (/could not claim/i.test(detail)) {
        message = 'USB device is claimed by something else. Unmount the camera and retry.';
      } else if (/unsupported operation|generic capture/i.test(detail)) {
        message = 'libgphoto2 is too old for EOS M capture, or the camera is in movie mode.';
    ...
    if (hadViewers) setTimeout(() => liveView.start(), 300);
  }
}
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (url.pathname === '/api/status') {
      const info = await withCamera('status', detect);
      return sendJson(res, 200, {
        ...info,
        liveView: liveView.state,
        frames: liveView.frameCount,
...
server.listen(PORT, '127.0.0.1', async () => {
  ...
  if (!SIMULATE) {
    ...
    if (info.cameraConnected) {
      log(`detected: ${info.model} on ${info.port}`);
    } else {
      warn('no camera detected yet; the bridge will keep serving and retry on demand');
    }
